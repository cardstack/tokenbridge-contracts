...
contract('HomeNftMediator', accounts => {
  const owner = accounts[0]
  const user = accounts[1]

  describe('Basic mediator', () => {
    beforeEach(async function() {
      this.bridge = await HomeNftMediator.new()
      this.mediatorContractOnOtherSide = ForeignNftMediator
  ...
  describe('deploy and register new token', () => {
    ...
    beforeEach(async () => {
      contract = await HomeNftMediator.new()
    ...
    describe('fixFailedMessage', () => {
      ...
      it('should allow updating tokenimage', async function() {
        ...
        const homeToken = await bridgedTokenContract(tokenOnForeign)

        const homeTokenAsUpdated = await UpdatedERC721ImageMock.at(homeToken.address)
        ...
        expect(await homeTokenAsUpdated.foo(tokenId)).to.be.equal(tokenURI)
