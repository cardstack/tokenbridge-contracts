...
const { expectEventInLogs, expectRevert } = require('../helpers/helpers')
...
contract('SimpleBridgeKitty', accounts => {
  ...
  describe('mint', () => {
    it('should mint with token Id and metadata', async () => {
      ...
      // Then
      await expectEventInLogs(logs, 'Birth', {
        owner: user,
  ...
  describe('burn', () => {
    it('should burn the kitty', async () => {
      ...
      await expectRevert(token.burn(kittyId, { from: user }))
      // only owner of kitty can burn
      await expectRevert(token.burn(kittyId, { from: owner }))
      ...
      // Then
      await expectEventInLogs(logs, 'Death', {
        kittyId: new BN(kittyId)
      })
      expect(await token.totalSupply()).to.be.bignumber.equal('0')
      await expectRevert(token.ownerOf(kittyId))
  ...
  describe('ERC721', () => {
    ...
    describe('approve', () => {
      it('should emit an approval event', async () => {
        ...
        await expectRevert(token.approve(user2, kittyId, { from: owner }))

        const { logs } = await token.approve(user2, kittyId, { from: user })

        // Then
        await expectEventInLogs(logs, 'Approval', {
          owner: user,
          approved: user2,
    ...
    describe('transfer', () => {
      it('should transfer ownership of the token', async () => {
        ...
        // When
        // should own the token
        await expectRevert(token.transfer(owner, kittyId, { from: owner }))
        // can't transfer to zero address
        await expectRevert(token.transfer(ZERO_ADDRESS, kittyId, { from: user }))
        // can't transfer to token address
        await expectRevert(token.transfer(token.address, kittyId, { from: user }))
        ...
        await expectEventInLogs(logs, 'Transfer', {
          from: user,
          to: owner,
          tokenId: new BN(kittyId)
        })
    ...
    describe('transferFrom', () => {
      it('should transfer ownership of a previously approved token', async () => {
        ...
        await expectRevert(token.transferFrom(user, owner, kittyId, { from: owner }))

        await token.approve(owner, kittyId, { from: user })

        // can't transfer to zero address
        await expectRevert(token.transferFrom(user, ZERO_ADDRESS, kittyId, { from: owner }))
        // can't transfer to token address
        await expectRevert(token.transferFrom(user, token.address, kittyId, { from: owner }))
  ...
  describe('tokenURI', () => {
    it('should return string URI of token', async () => {
      ...
      await expectRevert(token.tokenURI(1))
      ...
      await expectRevert(token.tokenURI(12345))
      ...
      await expectRevert(token.tokenURI(1714022))
